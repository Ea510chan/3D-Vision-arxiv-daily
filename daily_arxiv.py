...
def get_code_link(qword: str) -> str:
    """
    Search GitHub repositories by query string (title or arXiv ID).
    Returns the most starred repo URL, or None if not found.
    """
...
def get_daily_papers(topic, query="slam", max_results=2):
    ...
    for result in search_engine.results():
        ...
        try:
            ...
            # fallback 1: GitHub search by title
            if repo_url is None:
                repo_url = get_code_link(paper_title)
            # fallback 2: GitHub search by arXiv ID
            if repo_url is None:
            ...
            # optional: include comments if present
            if comments:
        ...
        for keywords, v in json_data.items():
            ...
            for paper_id, contents in v.items():
                ...
                try:
                    ...
                    r = requests.get(base_url + paper_id, timeout=10).json()
                    if "official" in r and r["official"]:
                        repo_url = r["official"]["url"]
